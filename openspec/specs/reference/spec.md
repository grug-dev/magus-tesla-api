...
The `reference` module holds external reference values. They belong to no vehicle and no user.
Today it holds one fact: the price of one gallon of regular gasoline per calendar month. A
month with no row of its own uses the newest earlier price.
