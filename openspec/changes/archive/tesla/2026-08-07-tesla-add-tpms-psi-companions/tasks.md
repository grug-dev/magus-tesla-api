...
- [x] 1.1 Add `barToPSI = 14.503773773` as a package-level constant in `internal/tesla/types.go`,
...
- [x] 1.2 Add `func (v VehicleStateTesla) TpmsPressureFLPSI() float64` returning
...
- [x] 1.3 Add the same companion for the front-right corner (`TpmsPressureFRPSI`).
- [x] 1.4 Add the same companion for the rear-left corner (`TpmsPressureRLPSI`).
- [x] 1.5 Add the same companion for the rear-right corner (`TpmsPressureRRPSI`).
- [x] 1.6 Confirm the return type is plain `float64` on all four (NOT `*float64`) and that the
...
- [x] 2.1 Add a unit test asserting each of the four companions returns the corner's bar value
...
- [x] 2.2 Add a unit test asserting a `0.0` bar reading converts to `0.0` PSI and is returned as an
      ordinary value (spec scenario "A truthfully reported zero bar converts to zero PSI").
- [x] 2.3 Verify no test added here calls any `Raw*` method or `cmd/explore-tesla-api` — the
      `tesla-exploration` capability must stay test-free (`CLAUDE.md`).

## 3. Docs

- [x] 3.1 Update `internal/tesla/AGENTS.md` where it describes the companion-method convention, so
...
- [x] 3.2 Confirm no doc change is needed in `cmd/explore-tesla-api/README.md` or
...
- [x] 4.1 `go build ./internal/tesla/...` and `go vet ./internal/tesla/...` pass.
- [x] 4.2 `go test ./internal/tesla/...` passes.
- [x] 4.3 `go build ./...` passes — this tier is additive, so unlike tiers 2 and 3 the full tree
      must still compile (RM7 Decision 8).
- [x] 4.4 `openspec validate tesla-add-tpms-psi-companions --strict` passes.
