...
- [ ] 1.1 Add the `SnapshotsByVehicleSince` query to `internal/telemetry/db/query.sql`: select the
...
- [ ] 1.2 Run `make sqlc` (or `sqlc generate`) to regenerate `telemetrydb`; confirm the generated
  method signature takes `(AccountID, TeslaID, Since)` params and returns the row slice.

## 2. Domain read port

- [ ] 2.1 Add `SnapshotsByVehicleSince(ctx context.Context, accountID uuid.UUID, teslaID int64,
...
- [ ] 2.2 Implement it in `internal/telemetry/reader.go`: call the generated query, map each
...
- [ ] 3.1 Add a one-line stub `SnapshotsByVehicleSince(...) ([]Snapshot, error) { return nil, nil }`
...
- [ ] 4.1 Unit test the mapping/empty contract with a fake `telemetrydb` querier (no DATABASE_URL):
  oldest-first ordering preserved, empty-non-nil on no rows, `account_id`/`tesla_id`/`since` passed
  through.
- [ ] 4.2 Extend the DATABASE_URL-gated store test (`internal/telemetry/db_read_integration_test.go`)
...
- [ ] 4.3 `make check` passes (build + vet + tests). No migration to apply.

## 5. Docs

- [ ] 5.1 No structural/module-surface doc change beyond the spec delta: this adds a method to an
