...
- [ ] 1.1 Rewrite the "**Miles → km conversion is mandatory**" bullet (`:44-47`) so it applies to
...
- [ ] 1.2 In the same bullet, delete the clause that forbids storing the converted value
...
- [ ] 1.3 Add an adjacent bullet stating the persisted-type rule: persisted domain types and
...
- [ ] 1.4 State the two exemptions inline in that bullet: vendor adapter DTOs (rule 1.1) and
...
- [ ] 2.1 Add a bullet to `ai/go-conventions.md` §Persistence requiring every unit-bearing column
...
- [ ] 2.2 State the negative half explicitly: identifiers, timestamps, dates, counts, names,
      states and flags take **no** suffix — so `captured_at`, `tesla_id` and
      `max_range_charge_counter` are not read as violations.

## 3. Update `CLAUDE.md`

- [ ] 3.1 Replace the "**Miles → km conversion is mandatory**" non-negotiable bullet with the
...
- [ ] 3.2 Keep the bullet short and point at `ai/go-conventions.md` for the full rule —
      `CLAUDE.md` is loaded on every request, so it carries the rule, not the rationale.
- [ ] 3.3 Verify the rewritten bullet does not contradict any other `CLAUDE.md` section,
      particularly the Performance-Profile line that this rule follows from.

## 4. Spread to the module `AGENTS.md` files

- [ ] 4.1 `internal/telemetry/AGENTS.md` — add the platform rule reference to its DTO/units
...
- [ ] 4.2 `internal/manualcharge/AGENTS.md` — add the rule plus the audit result for its table:
...
- [ ] 4.3 `internal/account/AGENTS.md` — add the rule and record that `accounts`, `tesla_tokens`
      and `vehicles` currently hold **no** unit-bearing columns, so the rule applies to future
      columns only.
- [ ] 4.4 `internal/tesla/AGENTS.md` — document the module as **the exception**: its DTOs stay in
...
- [ ] 4.5 Keep each `AGENTS.md` edit a short **pointer** to the rule plus that module's specific
      fact — not a full restatement. Five verbatim copies drift; that is the doc bloat the
      AI-efficiency rule warns against.
- [ ] 4.6 Confirm `internal/battery/AGENTS.md` and `internal/gateway/AGENTS.md` are **not**
...
- [ ] 5.3 Do **not** archive this change until RM7 tier 4 is archived. Until then
...
- [ ] 5.4 Re-run the audit as a final check: confirm no migration anywhere under
...
- [ ] 5.5 `openspec validate platform-unit-of-measure-convention --strict` passes.
- [ ] 5.6 Confirm `git diff --stat` for this change touches only `.md` files — no `.sql`, no
      `.go`, no `sqlc.yaml`.
