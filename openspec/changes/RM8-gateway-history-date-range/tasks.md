...
- [ ] 1.1 In `internal/gateway/handlers/history.go`, replace the `historyDayPresets` var, the
...
- [ ] 1.2 In `internal/gateway/templates/fragments/history_vm.go`, replace `HistoryView.Days int`
...
- [ ] 2.1 In `internal/gateway/handlers/history.go`, rewrite `DashboardHistoryFragment`: after the
      ...
      1.2.
- [ ] 2.2 Rewrite `buildHistoryView(ctx, uid, teslaID, start, end time.Time)` (drop the `days int`
...
- [ ] 2.3 Rewrite `buildOdometerChart(snaps, start, end time.Time)` (drop the `days int` param) to
...
- [ ] 2.4 Rewrite `buildBatteryChart(snaps, start, end time.Time)` (drop the `days int` param) to
...
- [ ] 2.5 Retarget `labelVerticalFor(numBars int) bool` (design D3): return `true` when
...
- [ ] 3.1 In `internal/gateway/templates/fragments/history.templ`, rewrite `historyDaysSelector`
...
- [ ] 3.2 In the same file, update `historyBarChart` to render a missing-day bar (`Present=false`)
...
- [ ] 3.3 In `internal/gateway/templates/pages/dashboard.templ`, **remove the Refresh button**
...
- [ ] 4.1 In `internal/gateway/AGENTS.md`, add a new "HTTP date-filter convention" section
...
- [ ] 4.2 In `ai/go-conventions.md`, add a one-line pointer (design D6): a new "HTTP date-filter
...
- [ ] 5.1 Run `make templ` (the pinned `go tool templ generate`) to regenerate
...
- [ ] 5.2 Run `make css` (Node-less Tailwind CLI) to regenerate `internal/gateway/static/app.css`
...
- [ ] 5.3 `make generate` (runs sqlc + templ + css together) may be used in place of 5.1+5.2 as a
...
- [ ] 6.1 Replace the `TestClampHistoryDays_*` tests with `TestParseHistoryRange_*` covering design
...
- [ ] 6.2 Replace the `fakeHistoryReader` stub's `SnapshotsByVehicleBetween` panic with a real
...
- [ ] 6.3 Add `TestBuildOdometerChart_FixedAxis_*` unit tests (design D3): (a) a 6-snap fixture for
...
- [ ] 6.4 Add `TestBuildBatteryChart_FixedAxis_*` unit tests (design D3): (a) a fixture for a 5-day
...
- [ ] 6.5 Add `TestBuildHistoryView_BothChartsShareFixedAxis` (the MAG-7 fix): for a fixture with
...
- [ ] 6.6 Rewrite the HTTP-level handler tests: `TestDashboardHistoryFragment_Default window`
...
- [ ] 6.7 Retarget the render/label tests: `TestDashboardHistoryFragment_LabelsMatchViewModelVerbatim`
...
- [ ] 6.8 Update `TestDashboard_HistoryRegionInsideDashboardContent` (the structural test rendering
...
- [ ] 7.1 Run `go test ./internal/gateway/...` and confirm the full gateway suite is green,
...
- [ ] 7.2 Run `go vet ./...` and `go build ./...` repo-wide; confirm no other `telemetry.Reader`
...
- [ ] 7.3 Confirm `git status` shows changes only under `internal/gateway/`,
      ...
      Depends on: 7.2.
- [ ] 7.4 Run the gateway AGENTS.md CI guard `make css && git diff --exit-code
