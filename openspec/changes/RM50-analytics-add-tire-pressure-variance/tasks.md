...
- [ ] A.1 Add four fields to `consumptionCalc` in `internal/analytics/consumption.go`:
      `TpmsPressureFLPSICalc`, `TpmsPressureFRPSICalc`, `TpmsPressureRLPSICalc`,
      `TpmsPressureRRPSICalc`, all `*float64`.
- [ ] A.2 Add a `tpmsDeltaPSI(prev, cur *float64) *float64` helper in
...
- [ ] A.3 Add the same four fields to `vehicleMetricRow` in
...
- [ ] A.4 Unit tests in `internal/analytics/consumption_test.go`: extend
...
- [ ] A.5 Unit tests in `internal/analytics/consumed_test.go`: extend
...
- [ ] B.2 Add `tpms_pressure_fl_psi_calc, tpms_pressure_fr_psi_calc,
...
- [ ] B.3 Add the same four columns to `LatestVehicleMetricsByAccount`'s SELECT column
...
- [ ] B.7 Add the four fields to `LatestMetricsByAccount`'s mapping loop in
      `internal/analytics/reader.go`, using the existing `ptrFloat64FromPg` helper.

- [ ] B.8 DB-integration test for the backfill migration, in
