...
- [ ] 1.1 In `mapVehicles` (`handlers.go:311,313`), change `snap.BatteryRangeKm()` and
...
- [ ] 1.2 In `mapVehicles` (`handlers.go:314-315`), adopt the renamed temperature fields
      (`snap.InsideTemp` → `snap.InsideTempC`, `snap.OutsideTemp` → `snap.OutsideTempC`).
- [ ] 1.3 In `mapDashboardSnapshot` (`handlers.go:400,405`), change `formatKm(snap.OdometerKm())`
      and `snap.BatteryRangeKm()` to field reads.
- [ ] 1.4 In `mapDashboardSnapshot` (`handlers.go:401-407`), adopt the other renamed fields:
      temperatures, `snap.BatteryLevel` → `snap.BatteryLevelPct`, `snap.ChargeLimitSoc` →
      `snap.ChargeLimitSocPct`.
- [ ] 1.5 Grep the whole module for any remaining renamed-field reference this list missed —
      `dashStatus`, `mergeSnapshots`, and the charges handlers also touch `telemetry.Snapshot`.
- [ ] 1.6 **Keep `formatKm`** (`handlers.go:425`) and its behavior (round to whole, group
...
- [ ] 1.7 Confirm no conversion factor (`1.609344`, `14.503773773`, `milesToKm`, `barToPSI`) is
      introduced anywhere in this module.

## 2. History charts (`history.go`)

- [ ] 2.1 In `buildOdometerChart` (`history.go:147`), change
      `pts[i].OdometerKm() - pts[i-1].OdometerKm()` to the field reads.
- [ ] 2.2 In `buildOdometerChart` (`history.go:154`), change `odometerKm: pts[i].OdometerKm()` to
      the field read.
- [ ] 2.3 In `buildBatteryChart` (`history.go:196`), change `formatKmRaw(s.BatteryRangeKm())` to
      the field read.
- [ ] 2.4 Adopt the renamed battery-level field wherever the battery chart reads it.
- [ ] 2.5 Confirm the per-day delta arithmetic, the negative-delta-clamps-to-zero rule, the bar
...
- [ ] 2.6 Keep `formatKmRaw` (`history.go:206`) — like `formatKm` it only rounds and never
      converted.

## 3. Tests

- [ ] 3.1 Update every `telemetry.Snapshot` literal in `handlers_test.go` and `history_test.go` to
      the renamed fields.
- [ ] 3.2 **Fix fixtures whose values are written in miles** (design D-risk):
      ...
      miles→km comments.
- [ ] 3.3 Replace expectations computed by calling a deleted method — e.g.
...
- [ ] 3.4 Keep the `formatKm` unit tests at `handlers_test.go:677-686` passing unchanged — that
      helper's behavior is not modified by this tier.
- [ ] 3.5 Verify the dashboard/vehicle-card assertions still expect the same rendered strings as
...
- [ ] 4.1 Update `internal/gateway/AGENTS.md:188`, which states the template does "no
...
- [ ] 4.2 Confirm no other gateway doc, template, or view-model comment references a companion
      method or describes a stored value as being in miles.

## 5. Verification gate (FULL TREE — this is RM7's final gate)

- [ ] 5.1 `go build ./...` passes for the whole repository — the build breakage opened by RM7
      tier 2 is closed here (RM7 Decision 8).
- [ ] 5.2 `go vet ./...` and `go test ./...` pass.
- [ ] 5.3 `make check` passes.
- [ ] 5.4 Run the app against a migrated database and compare the `/dashboard` odometer, range and
      ...
      what the user sees.
- [ ] 5.5 `grep -rn "OdometerKm()\|BatteryRangeKm()\|TpmsPressure.*PSI()" internal/gateway internal/battery`
      returns nothing, and the same grep across `internal/` returns only the `internal/tesla`
      adapter definitions.
- [ ] 5.6 `openspec validate gateway-adopt-snapshot-unit-fields --strict` passes.
