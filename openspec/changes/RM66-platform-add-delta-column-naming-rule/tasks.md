...
- [x] T1.1 Before writing the target, re-run design.md's two measurement greps
...
- [x] T1.2 Add a `delta-guard:` target after `naming-guard:`, mirroring its
...
- [x] T1.3 Add `delta-guard` to the `.PHONY` list, in the existing guard cluster.
- [x] T1.4 Update `check:` to insert `delta-guard` immediately before `test`
...
- [x] T2.1 Add the delta-column naming rule as its own bullet beside the
...
- [x] T3.1 `README.md`'s `make check` composition comment (`README.md:93-94`):
...
- [x] T3.2 `CLAUDE.md` "Builds & local checks — PROJECT OVERRIDE" section: add
      `make delta-guard` to the allowed-guards bullet, and to the "its other
      phases" sentence's guard list.
- [x] T3.3 `CLAUDE.md` "Pipeline config" §Test-Execution-Policy block: add
...
- [x] T4.1 Run `make delta-guard` on the current tree (after T1 lands). MUST
      pass — zero failures, exit 0 (warnings for the baseline names are
      expected and fine). Record the exact output.
- [x] T4.2 Prove the guard detects a violation, in both directions (design.md
      ...
      both outputs.
- [x] T4.3 `go build ./...`, `go vet ./...`, `gofmt -l` — confirm no diff and no
...
- [x] T4.4 `openspec validate RM66-platform-add-delta-column-naming-rule --strict`
      passes.
- [x] T4.5 Confirm no database object was touched: `git status` shows no file
