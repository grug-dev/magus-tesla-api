...
- [ ] B.1 `internal/gateway/handlers/supercharger.go:274` (`buildSuperchargerRows`, the
  session `CostLabel`) — replace `fmt.Sprintf("%.2f %s", *s.TotalCost, *s.Currency)` with
  `formatMoney(*s.TotalCost, *s.Currency)`.
- [ ] B.2 `internal/gateway/handlers/charges.go:540` (`chargeEntryVMFromEntry`, `PriceLabel`)
  — replace `fmt.Sprintf("%.2f %s", e.Price, e.Currency)` with
  `formatMoney(e.Price, e.Currency)`.
- [ ] B.3 `internal/gateway/handlers/charges.go:480` (`chargeEntryVMFromEntry`,
...
- [ ] B.4 **Do NOT touch** `internal/gateway/handlers/charges.go:550-551`
...
- [ ] C.1 Add `internal/gateway/handlers/format_test.go` with a table-driven `TestFormatMoney`
  covering at minimum: `0 -> "0.00 COP"`, `-1500.5 -> "-1,500.50 COP"`, `999 -> "999.00 COP"`,
  `1000 -> "1,000.00 COP"`, `58000 -> "58,000.00 COP"`.
- [ ] C.2 In `internal/gateway/handlers/charges_test.go`, add or extend the existing
...
- [ ] C.3 In `internal/gateway/handlers/supercharger_test.go`, add or extend a test asserting
...
- [ ] C.4 Add a test (in `charges_test.go` or a dedicated case) asserting `RawEnergyKWh` and
...
- [ ] D.1 Add a `money-guard` target to the `Makefile`, mirroring `ui-guard`/`i18n-guard`'s
...
- [ ] D.2 Add `money-guard` to the `.PHONY` list (Makefile line ~62, alongside
  `ui-guard i18n-guard`).
- [ ] D.3 Add `money-guard` to the `check` target's prerequisite list: `check: build vet
  ui-guard i18n-guard money-guard test`, and update `check`'s `##` help text to mention it.
- [ ] D.4 Run `make money-guard` against the repo AFTER task B lands and confirm it passes
...
- [ ] E.1 If `internal/gateway/AGENTS.md`'s "How to add or modify a page" recipe or any other
...
- [ ] E.2 No root `README.md` "Project Structure"/"Architecture" table edit — no module was
