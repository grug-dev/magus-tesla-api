...
- [ ] A.1 Add `HistoryView` / `HistoryChart` / `HistoryBar` view-model types in
...
- [ ] A.2 Add `historyDayPresets = []int{6, 14, 30}` and `defaultHistoryDays = 6` as named handler
  constants; add a `clampHistoryDays(raw string) int` helper (parse, reject non-preset → default).
- [ ] A.3 Add `DashboardHistoryFragment(c *gin.Context)` in `internal/gateway/handlers/`: auth
...
- [ ] A.4 Build the view model from the snapshots: **odometer** = last `days+1` points → `days`
...
- [ ] A.5 Register `GET /ui/dashboard/history` in `internal/gateway/gateway.go` behind the same auth
  middleware as the other `/ui/*` fragments.

## B. Templates (Templ)

- [ ] B.1 Add a reusable `historyBarChart(chart fragments.HistoryChart, colorClass string)` Templ
...
- [ ] B.2 Add a `historyDaysSelector(active int, presets []int)` Templ component: a DaisyUI `join`
...
- [ ] B.3 Add a `dashboardHistory(v fragments.HistoryView)` block component that renders the
...
- [ ] C.1 In `internal/gateway/templates/pages/dashboard.templ`, replace the two
...
- [ ] C.2 Run `make templ` (or the project's Templ generate step) so `*_templ.go` is regenerated;
  confirm `make check`'s ui-guard passes (no hardcoded colours, semantic tokens only).

## D. Docs + AGENTS.md decision (RD8)

- [ ] D.1 `internal/gateway/AGENTS.md`: add the "Charts are hand-rolled SVG — no chart library"
...
- [ ] D.2 If `ai/htmx-conventions.md` documents chart/data-viz patterns, add a one-line pointer to
  the SVG-bar-chart pattern (self-loading region + `<title>` tooltips); otherwise skip (no
  invented section).
- [ ] D.3 No root README structure-tree change (no module added/removed/renamed). Confirm the
  dashboard description in any gateway `README.md` mentions the history charts if it enumerates the
  bento cards.

## E. Tests + verification

- [ ] E.1 Handler tests (fake `telemetry.Reader`): `days` clamp (missing/invalid/out-of-set → 6;
...
- [ ] E.2 A render test asserting the fragment contains the responsive `<svg viewBox` + `<title>`
...
- [ ] E.3 `make check` passes (build + vet + ui-guard + tests). No DB, no migration.
