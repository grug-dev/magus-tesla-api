...
- [ ] T2.1 Qualify every table reference with `charging.` across all 10 `-- name:` blocks:
...
- [ ] T2.2 Rename the two `-- name:` lines that embed the old table name (design.md
...
- [ ] T2b.1 Find every hand-written SQL statement in this module's `_test.go` files that
...
- [ ] T2b.2 `db_session_reader_by_vehicle_integration_test.go`'s T-Order2 case asserts literal
...
- [ ] T3.1 Add a `rename:` map under the charging entry's existing `gen.go` block in the root
...
- [ ] T3.3 **Diff `internal/charging/db/models.go` against its pre-change version and confirm
...
- [ ] T3.4 Confirm by inspection that `internal/charging/db/query.sql.go` compiles against the
...
- [ ] T3b.1 `internal/charging/session_writer.go`: update `qtx.MirrorChargeSession(ctx,
...
- [ ] T3b.2 `internal/charging/session_verifier.go`: update `q.VerifyChargeSession(ctx,
...
- [ ] T3b.3 `internal/charging/session_reader.go`: `rowToSession` takes a
...
- [ ] T4.1 Update `internal/charging/AGENTS.md`'s Data Ownership section: state that this
...
- [ ] T4.2 Update `internal/charging/charging.go`'s `SuperchargerSessionAnalyticsReader` doc
...
- [ ] T4.3 Grep the repo (`grep -rln` for `charge_sessions`, `ChargeSession`, scoped to
