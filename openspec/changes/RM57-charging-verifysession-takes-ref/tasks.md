...
- [x] 1.1 `internal/charging/charging.go`: add
      `"github.com/cristianpena/magus-tesla-api/internal/vehicleref"` to the import
      block.
- [x] 1.2 Same file: change `SessionVerifier.VerifySession`'s signature from
...
- [x] 1.3 Same file: replace the `id/teslaID scope the update` doc-comment paragraph
      with the `id/ref scope the update` text `design.md` D3 gives verbatim.
- [x] 1.4 Same file: replace `Calling VerifySession(ctx, teslaID, id, nil, nil)` with
      `Calling VerifySession(ctx, ref, id, nil, nil)` in the doc comment a few lines
      above (D3).
- [x] 1.5 Do NOT change `SessionReader`, `SuperchargerSessionAnalyticsReader`, or any
...
- [x] 2.1 `internal/charging/session_verifier.go`: add
      `"github.com/cristianpena/magus-tesla-api/internal/vehicleref"` to the import
      block.
- [x] 2.2 Same file: change `VerifySession`'s signature to match T1.2, and add
      ...
      point.
- [x] 2.3 Same file: correct the comment currently reading
      `// the teslaID parameter above -- it is NOT NULL...` to
      `// the teslaID local variable above -- it is NOT NULL...` (it is no longer a
      parameter).
- [x] 2.4 `go build ./internal/charging/...` — expect clean (test files still fail
...
- [x] 3.1 `internal/charging/db_session_verifier_integration_test.go`: add
      `"github.com/cristianpena/magus-tesla-api/internal/vehicleref"` to the import
      block.
- [x] 3.2 Same file, next to `ptrIntV`: add the `refFor` helper exactly as
...
- [x] 3.3 Do NOT add a second `refFor`-shaped helper in any other file. All five test
...
- [x] 4.1 `db_session_verifier_integration_test.go` — 20 call sites, current lines
...
- [x] 4.2 `db_inferred_capacity_sessions_integration_test.go` — 2 call sites, current
      lines 137 and 210. Same substitution.
- [x] 4.3 `db_session_mirror_change_detection_integration_test.go` — 2 call sites,
      current lines 242 and 301. Same substitution.
- [x] 4.4 `db_monthly_capacity_integration_test.go` — 5 call sites, current lines
      274, 449, 639, 785, 803. Same substitution.
- [x] 4.5 `db_session_reader_updated_since_integration_test.go` — 1 call site,
      current line 154. Same substitution.
- [x] 4.6 After 4.1–4.5, grep the module for `VerifySession(ctx, ` followed
      immediately by a bare identifier or digit (not `refFor(` or `ref`) — expect
      zero hits inside `internal/charging`.
- [x] 4.7 Do NOT change any expected value, assertion, or seeded fixture in any of
      these five files — `design.md` D6 is explicit that no test's expected outcome
      changes, only its call syntax.
- [x] 4.8 `go vet ./internal/charging/...` and `gofmt -l internal/charging` — expect
...
- [x] 5.1 `internal/charging/AGENTS.md` §Allowed Imports: add
...
- [x] 5.2 Same file, §"The Supercharger session verification port": add one sentence
...
- [x] 5.3 `kkpa/context/workflows/supercharger-stats-read.md` line 45: update the
...
- [x] 5.4 `kkpa/context/use-case/charging/verify-session-battery.md` line 47: update
...
- [x] 5.5 Do NOT edit `kkpa/context/architecture/gateway-reader-writer-ports.md` or
...
- [x] 5.6 Do NOT edit anything under `openspec/changes/archive/` or
...
- [x] 6.1 `go build ./internal/charging/...`
- [x] 6.2 `go vet ./internal/charging/...`
- [x] 6.3 `gofmt -l internal/charging`
- [x] 6.4 `make vehicleref-guard` — expect clean. `refFor`'s `vehicleref.All(` call
      needs no marker; the guard already exempts `_test.go` files (see T3.2).
- [x] 6.5 `make boundary-guard` — expect clean and unaffected (this change touches
      neither `internal/gateway` nor `internal/telemetry`).
- [x] 6.6 `make archive-guard` — expect clean.
- [x] 6.7 `go build ./...` (whole repo) — expect a failure confined to
...
- [x] 6.8 `openspec validate RM57-charging-verifysession-takes-ref --strict`
- [x] 6.9 Hand the owner the suite command this agent never runs:
