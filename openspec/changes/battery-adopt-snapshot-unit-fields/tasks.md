...
- [ ] 1.1 In `internal/battery/derive.go:32`, change
      `end.OdometerKm() - start.OdometerKm()` to `end.OdometerKm - start.OdometerKm`.
- [ ] 1.2 In `internal/battery/derive.go:54-55`, change `FromKm: start.OdometerKm()` and
      `ToKm: end.OdometerKm()` to the field reads.
- [ ] 1.3 Update any other `telemetry.Snapshot` field reference in the module that tier 2 renamed
...
- [ ] 1.4 Confirm no arithmetic constant (`1.609344`, `milesToKm`, or any literal factor) is
      introduced anywhere in this module — after this tier it must contain zero unit conversion.
- [ ] 1.5 Confirm the `Efficiency` type is unchanged: `WhPerKm`, `FromKm`, `ToKm`,
      `BatteryDeltaPct`, `Approximate` keep their names, types and meanings (design D3).

## 2. Tests

- [ ] 2.1 Update every `telemetry.Snapshot` literal in `derive_test.go` and `reader_test.go` to the
      renamed fields.
- [ ] 2.2 **Review each fixture for meaning, not just field name** (design D2): a fixture that
...
- [ ] 2.3 Add or adapt one test with a hand-computed expected Wh/km from a known kilometre distance
...
- [ ] 2.4 Confirm the existing `ok=false` tests (fewer than two snapshots, no distance moved, net
      charge exceeds consumption) still exercise the same conditions after the fixture edits.

## 3. Docs

- [ ] 3.1 Update `internal/battery/AGENTS.md:48-50`, which describes `FromKm`/`ToKm` as
...
- [ ] 4.1 `go build ./internal/battery/...` and `go vet ./internal/battery/...` pass, on a tree
      with RM7 tier 2 applied.
- [ ] 4.2 `go test ./internal/battery/...` passes.
- [ ] 4.3 `grep -rn "OdometerKm()\|BatteryRangeKm()\|1.609344\|milesToKm" internal/battery` returns
      nothing.
- [ ] 4.4 Do NOT run `go build ./...` as this tier's gate — `internal/gateway` is still expected to
      fail until RM7 tier 4 lands (RM7 Decision 8).
- [ ] 4.5 `openspec validate battery-adopt-snapshot-unit-fields --strict` passes.
