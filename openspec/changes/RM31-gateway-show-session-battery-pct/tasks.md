...
- [ ] **4.1** **[module: gateway worker]** Re-read the implemented diff against design.md D1–D6 and specs/gateway/spec.md, then update only the completed task checkboxes in this file. Acceptance: no code outside `internal/gateway/`, no database object, write path, estimator, Country restoration, or new chart implementation has entered the change. `depends_on`: 3.1, 3.2, 3.3 · `parallel_ok`: no.
