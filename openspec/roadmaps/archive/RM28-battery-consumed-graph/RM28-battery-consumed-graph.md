...
| **D17** | **D8's "start day" means the row's own `EffectiveDate`** — which, for a multi-day span, is the span's **last** calendar day. Confirms design **D-B3** as written. | Placing the bar on the span's *first* day would require `prev.EffectiveDate + 1` arithmetic — the very shift **D1**'s correction removed. The intervening days render as no-data either way, so the cheaper reading loses nothing. *(Owner, 2026-08-16.)* |
| **D18** | **D6 STANDS LITERALLY — bucketing uses `Config.Location`, not UTC.** Supersedes design **D-B7**'s UTC-`EffectiveDate` reading. | Owner's call, made against the leader's UTC recommendation and with the consequence stated and accepted: `internal/battery` buckets in UTC−5 while the gateway's existing odometer/battery charts still bucket in UTC via `effectiveDayUTC`. Tier 3 buckets in Bogota; **tier 4 inherits the mismatch** and must key on `DayConsumption.Date` directly rather than re-bucketing. *(Owner, 2026-08-16.)* |
| **D18a** | **The zone reaches `internal/battery` inside the data, not through injection.** The bucket day is `CapturedDate − 1 day`; `EffectiveDate` is never read by this module. `NewReader`'s signature is unchanged. | `CapturedDate` is *already* stamped in `Config.Location` on the write path (`telemetry/service.go:510`, `dateOnly(capturedAt, loc)`) — the same column **D6** points at. `EffectiveDate` is `CapturedAt − 1 day` computed in **UTC**, and a bare date has no instant left to re-project. Injecting a `*time.Location` was rejected: it would make the zone a per-composition-root input, letting `cmd/web` and `cmd/poller` disagree silently — the exact two-clocks failure D6 exists to prevent. *(Worker-derived under D18, leader-verified 2026-08-16.)* |
| **D19** | **The consumed chart scales RELATIVE to the window max**, mirroring `buildOdometerChart` — tallest bar = 100% of the canvas — *not* absolute 0–100 like `buildBatteryChart`. | A typical 6–20 %/day range would leave an absolute 0–100 axis nearly empty and unreadable. The cost is that bar heights are not comparable between two different windows; the tooltip carries the real percentage, so no information is lost. *(Owner, 2026-08-16.)* |
| **D20** | **A multi-day span (`DaysSpanned > 1`) plots its REAL `ConsumedPct`** with its **own** distinct marker and a "covers N days" tooltip — a separate visual vocabulary from **D10**'s gap marker, never a zero-height bar. | The value is genuinely observed, just over a longer window. Zeroing it would discard real consumption — exactly what **D8** rejects — and would make a missed poll indistinguishable from a data error. **D10**'s zero-height bar stays reserved for the flagged days, where the number is one we do not believe. *(Owner, 2026-08-16.)* |
| **D21** | **A day that is BOTH flagged (D10) and a multi-day span (D20) carries BOTH markers**, with both facts in the tooltip. Markers are therefore a **set**, not a single-valued enum. Resolves design **D-G4**, which had proposed span-marker-only. | The overlap is reachable — `Flagged` and `DaysSpanned` are independent, and a spanned row's summed `ConsumedPct` can be negative. **The height question is moot**: `Flagged` is true only when `ConsumedPct < 0` or `== 0` (D5), so `max(0, ConsumedPct)` is `0` under *either* reading. The only real choice was which marker shows, and picking one hides a fact that is true: the poll missed days **and** a charge record is missing. *(Owner, 2026-08-16.)* |
