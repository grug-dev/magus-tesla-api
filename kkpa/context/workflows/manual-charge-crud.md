...
| `internal/charging/db/query.sql` | sqlc source of truth: `CreateEntry`, `UpdateEntry` (`:one`, `WHERE id AND created_by_account_id`), `DeleteEntry`, list queries. The `created_by_account_id` guard on `Update`/`Delete` is transitional (`RM58-charging-demote-manual-charge-account-id`): it matches who typed the entry, not the entry's vehicle, until a later tier re-keys it onto `tesla_id`. |
...
- **Update:** `ExternalChargeRowUpdate` — guards as above → `fetchEntryTeslaIDAndChargedOn` FIRST (captures the OLD `ChargedOn`; unrecoverable after commit) → `chargingWriter.Update` (double-scoped `WHERE id AND created_by_account_id`, transitional — matches the typing account, not yet the vehicle; immutable `tesla_id`/`vin`/`created_at` untouched) → `recalculateAfterExternalChargeWrite(new ChargedOn)` AND, if the date changed, again for `oldChargedOn` → render static `ExternalChargeRow`.
