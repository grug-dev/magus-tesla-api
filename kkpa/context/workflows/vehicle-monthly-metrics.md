...
| `internal/gateway/handlers/vehicle_stats_chart.go` | The month-by-month bar chart, and `vehicleStatsPrevWindow` — the preceding equal-length period the KPI arrows compare against. The chart's axis is **calendar-driven**: a month with no stored row keeps its labelled slot at zero height, because this table is sparse by month and hiding the gap would draw a year that was never recorded. |
| `internal/gateway/handlers/vehicle_stats_gaps.go` | The missing-charge-records warning. Reads `Reader.ConsumedByDay`'s live `flagged`, **never `charge_gaps`** — that worklist keeps rows for gaps resolved more than 30 days ago (2 of 6 on this repo's data). |
