...
| File | Role |
|---|---|
| `internal/analytics/analytics.go` | Port declarations: `Recalculator` (`Recalculate`, `Reconcile`), `GapReconciliationWindow` (30d), `Reader` (reads `vehicle_metrics` — `ConsumedByDay`, `OdometerDeltaByDay`, `BatteryLevelByDay`, `LatestMetricsForVehicles`), `GapWriter`, `MonthlySyncer`, and `MonthlyReader` (the per-month rollup, a different table — `workflows/vehicle-monthly-metrics.md`). |
...
- **Read the metrics:** `analytics.Reader` — `ConsumedByDay`, `OdometerDeltaByDay`, `BatteryLevelByDay`, `LatestMetricsForVehicles`. Callers read the PORT, never `vehicle_metrics` directly. Two consumers today: the gateway's dashboard history fragment (all four), and `/vehicle-stats`' missing-charge-records warning, which reads `ConsumedByDay` for its **`Flagged` + `MissingChargingType`** rather than its figures (MAG-87) — that warning must read this live flag and **never** `analytics.charge_gaps`, which keeps rows for gaps resolved more than 30 days ago. The per-MONTH rollup is a separate port, `analytics.MonthlyReader` over `vehicle_monthly_metrics` — see `workflows/vehicle-monthly-metrics.md`.
