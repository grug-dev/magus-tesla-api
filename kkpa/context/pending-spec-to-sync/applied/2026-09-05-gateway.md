...
## APPLY-TIME CORRECTION (2026-09-05)

The first `Conventions & gotchas` bullet ("this page's status badge deliberately has NO
`ui.Dot`") was **NOT applied as drafted — it had gone stale between staging and apply.**
`internal/gateway/templates/fragments/supercharger_row.templ` pairs a `ui.Dot` with the
`ui.Badge` in all three states; MAG-46 step 4.2 added it after this proposal was written,
because the badge is shrunk rather than hidden on a phone and `title` tooltips never fire on
touch, leaving colour as the sole carrier of meaning.

Applied instead: a bullet recording what actually shipped, noting that the spec's no-Dot
sentence is superseded, and that the Dot (`success`/`neutral`/`warning`) and the Badge
(`primary`/`neutral`/`ghost`) use different vocabularies on purpose.

Bullets 2 and 3 were applied verbatim; both were verified against the code (the edit form
carries no status field; `colspan="8"` is present in both `supercharger_row.templ` and
`supercharger_row_edit.templ`). The INDEX row was extended to name the Dot alongside the Badge.
