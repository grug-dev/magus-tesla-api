...
RM59 tier 1 re-keyed `vehicle_metrics` and `vehicle_metric_watermarks` on `tesla_id` and dropped
`account_id`. Three requirements were modified: **Multi-Tenant Scoping On Every Underlying Read**,
**Latest Vehicle Status Per Account**, and **Per-Day Battery Level and Range Read**.

The guide's `## Conventions & gotchas` carried two bullets that the new spec makes false:

1. "An account with no computed rows returns an empty result" — the input is a vehicle set now.
2. "Every per-day battery read is scoped to the requesting account's own vehicle" — it is scoped by
   vehicle identity alone. The account check moved out of this capability.

Two bullets are added for the new invariants. Everything else in the section is carried over
unchanged, which is why this is a REPLACE and not an APPEND.

No `## Component map` block: a spec carries behavior, not file paths, and the T7 doc task already
updated that section. No `[index]` block: the routing rows already name `LatestMetricsForVehicles`.
...
- **This capability takes no account identifier anywhere, and it never checks ownership.** Every read and every recompute is scoped by vehicle identity alone. Proving that the requesting account may see a vehicle happens BEFORE the call, in the caller. Adding an account parameter back to any port here is a regression, not a hardening.
  _Source: spec analytics — Requirement: Multi-Tenant Scoping On Every Underlying Read; Requirement: Latest Vehicle Status Per Account._
