...
## Applied — what actually changed (2026-09-14)

Verified against the code before writing, and the check found **one more stale bullet the
proposal missed**. Both were applied.

- `workflows/supercharger-stats-read.md` — 6 bullets appended to `## Conventions & gotchas`;
  the `NULL TeslaID` bullet deleted; the orphan-recovery bullet **rewritten** (it told the
  reader that switching to the per-vehicle port would break orphan recovery — `processor.go`
  already uses `SuperchargerHistoryByVehicleUpdatedSince`, and the account-wide port is gone);
  the backfill bullet re-worded from account to vehicle.
- `INDEX.md` — the two `mirror_watermarks` / `mirror watermark` rows now say per-vehicle;
  3 rows added to `## Architecture topics`.

**Not fixed here, reported instead:** `openspec/specs/charge-session-log/spec.md` is itself
stale — its requirement is still titled "Bounded By An Account Watermark" and it still carries
the scenario "A session for a currently-unregistered vehicle is still recovered". That is an
OpenSpec spec, not the KB, so it needs its own change.
