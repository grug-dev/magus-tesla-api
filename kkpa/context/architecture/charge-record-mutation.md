...
| `km_per_pct_calc`, `efficiency_range_100_pct_km_calc` (named `estimated_range_km_calc` until MAG-79) | distance ÷ **`consumed_pct`**, NULL unless `consumed_pct > 0` — MAG-81 changed this divisor from the raw `battery_used_pct_calc`, which NULLed every day the vehicle drove AND charged | `analytics/consumption.go` `deriveConsumption` | `vehicle_metrics` |
...
- **The manual pre-write lookup is capped at 100 rows** — `fetchEntryTeslaIDAndChargedOn` and
  `fetchEntryVM` call `ListEntriesByVehicles(ctx, vehicleref.TeslaIDs(refs), 0)`, where `refs`
  and `vehicles` both come from one `h.ownedVehicles(ctx, uid)` call (itself backed by
