...
|---|---|---|
| ~~`internal/gateway/**`~~ — **NO LONGER A CONSUMER, and now forbidden** | — | The gateway's four snapshot call sites were repointed onto `analytics.Reader` by **RM38** (`LatestMetricsForVehicles` — dashboard, vehicle cards, nav header, charges battery suggestion) and **RM40** (`BatteryLevelByDay` — the history battery chart). The supercharger page had already moved to `charging.SessionReader` in RM30. `make boundary-guard` now fails the build on any `internal/telemetry` import under `internal/gateway/`, with **zero** `// boundary:allow:` escape hatches. See the boundary gotcha below. |
...
- **Same-day captures dedupe** — `UNIQUE (tesla_id, captured_date)`, latest wins; repeated same-day collection is not duplicate data. _Source: `telemetry-rekey-vehicle-snapshots-on-tesla-id` design D-MIGRATION._
