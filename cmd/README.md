...
| `cmd/monthly-capacity` | `go run ./cmd/monthly-capacity` / `make cmd-monthly-capacity` | Measures one month's effective pack capacity, for one vehicle or every vehicle. Local only, run by hand — not one of the two containerized binaries above. | Run on-demand, exits |
