...
| `cmd/poller` | `go run ./cmd/poller` | Nightly telemetry collection (scheduled), then per-vehicle reconciliation — analytics' `vehicle_metrics` first, charge gaps second (that order is required: gap detection now reads the metrics); `--once` runs a single cycle and exits | Long-running nightly, or one-shot with `--once` |
