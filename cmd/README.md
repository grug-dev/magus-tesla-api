...
| `cmd/migrate` | `go run ./cmd/migrate` | Applies all four modules' goose migrations, in order, then exits. Uses the goose library directly (not the goose CLI — see the file's doc comment for why). It is the `migrate` service's `ENTRYPOINT` in the Docker deploy. | Run once per deploy, exits |
