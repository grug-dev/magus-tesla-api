...
// parseChargesRangeAt builds a gin.Context with the given start/end query
// params and runs parseChargesRange against the given explicit today —
// mirrors history_test.go's parseRange / supercharger_test.go's
// parseSuperchargerRangeAt helpers, but for parseChargesRange's (c, today)
// signature.
func parseChargesRangeAt(start, end string, today time.Time) (time.Time, time.Time, bool) {
	...
	return parseChargesRange(c, today)
}

// --- parseChargesRange / buildChargesPresets Test Contract Group A
// (design.md §Test Contract Group A, RM33-gateway-add-entries-dashboard) ---

// TestParseChargesRange_A1_BothAbsent_DefaultSevenDayWindow is Test Contract
...
// today, NOT today-1).
func TestParseChargesRange_A1_BothAbsent_DefaultSevenDayWindow(t *testing.T) {
	today := time.Date(2026, 8, 29, 0, 0, 0, 0, time.UTC)
	start, end, ok := parseChargesRangeAt("", "", today)
	if !ok {
...
// TestParseChargesRange_A2_ValidExplicitWindow is Test Contract A2: a valid,
// non-default window is always accepted, not just the two presets.
func TestParseChargesRange_A2_ValidExplicitWindow(t *testing.T) {
	today := time.Date(2026, 8, 15, 0, 0, 0, 0, time.UTC)
	start, end, ok := parseChargesRangeAt("2026-08-01", "2026-08-31", today)
	if !ok {
...
// TestParseChargesRange_A3_FutureEndAccepted is Test Contract A3: an `end`
...
// (start absent, end present), which parseChargesRange's own frozen body
...
func TestParseChargesRange_A3_FutureEndAccepted(t *testing.T) {
	today := time.Date(2026, 8, 29, 0, 0, 0, 0, time.UTC)
	start, end, ok := parseChargesRangeAt("2026-08-25", "2026-09-15", today)
	if !ok {
...
// TestParseChargesRange_A4_PartialPair is Test Contract A4: start present
// without end (and vice versa) is rejected.
func TestParseChargesRange_A4_PartialPair(t *testing.T) {
	today := time.Date(2026, 8, 29, 0, 0, 0, 0, time.UTC)
	if _, _, ok := parseChargesRangeAt("2026-08-01", "", today); ok {
		t.Error("want ok=false when start present but end absent")
	}
	if _, _, ok := parseChargesRangeAt("", "2026-08-31", today); ok {
		t.Error("want ok=false when end present but start absent")
	}
}

// TestParseChargesRange_A5_EndBeforeStart is Test Contract A5.
func TestParseChargesRange_A5_EndBeforeStart(t *testing.T) {
	today := time.Date(2026, 8, 29, 0, 0, 0, 0, time.UTC)
	if _, _, ok := parseChargesRangeAt("2026-09-01", "2026-08-01", today); ok {
		t.Error("want ok=false when end < start")
	}
}

// TestParseChargesRange_A6_MaxDaysCap is Test Contract A6: a window wider
...
// TestParseSuperchargerRange_MaxDaysCap uses (chargesRangeMaxDays == 400 ==
// superchargerRangeMaxDays, D-Range).
func TestParseChargesRange_A6_MaxDaysCap(t *testing.T) {
	today := time.Date(2026, 8, 27, 0, 0, 0, 0, time.UTC)
	if _, _, ok := parseChargesRangeAt("2025-07-01", "2026-08-06", today); ok {
		t.Error("want ok=false for a window wider than the 400-day cap")
	}
	if _, _, ok := parseChargesRangeAt("2025-07-01", "2026-08-05", today); !ok {
		t.Error("want ok=true for a window exactly 400 days wide")
	}
}

// TestParseChargesRange_A7_ThisMonthIsFullCalendarMonth is Test Contract A7:
// buildChargesPresets with today=2026-08-15 (a 31-day month) -> "This
// month"'s EndStr == "2026-08-31" (the LAST day, not "2026-08-15"),
// StartStr == "2026-08-01".
func TestParseChargesRange_A7_ThisMonthIsFullCalendarMonth(t *testing.T) {
	today := time.Date(2026, 8, 15, 0, 0, 0, 0, time.UTC)
	start := today.AddDate(0, 0, -(chargesRangeDefaultDays - 1))
	presets := buildChargesPresets(context.Background(), start, today, today)
	if len(presets) != 2 {
		t.Fatalf("want 2 presets, got %d", len(presets))
	}
	// Index 1 is "this month" by buildChargesPresets's fixed return order
...
// TestParseChargesRange_A8_ActiveComputedByExactMatch is Test Contract A8:
// buildChargesPresets called with (start,end) exactly matching the "last 7
...
// Active==false.
func TestParseChargesRange_A8_ActiveComputedByExactMatch(t *testing.T) {
	today := time.Date(2026, 8, 15, 0, 0, 0, 0, time.UTC)

	// last-7-days window.
	last7Start := today.AddDate(0, 0, -(chargesRangeDefaultDays - 1))
	presets := buildChargesPresets(context.Background(), last7Start, today, today)
	if len(presets) != 2 {
		t.Fatalf("want 2 presets, got %d", len(presets))
	}
	if !presets[0].Active {
	...
	presets = buildChargesPresets(context.Background(), customStart, customEnd, today)
