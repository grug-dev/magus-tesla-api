// charges_range.go contains the Charge log page's date-filter range parser
...
// chargesRangeDefaultDays is the default window size (in days) applied when
// both ?start=&end= params are absent (design.md §D-Range).
const chargesRangeDefaultDays = 7

// chargesRangeMaxDays is the hard cap on the window width, protecting the
...
// parseChargesRange validates the ?start=&end= query params for
// GET /ui/charges/list (design.md §D-Range, verbatim). today is the caller's
...
func parseChargesRange(c *gin.Context, today time.Time) (start, end time.Time, ok bool) {
	...
	if e.Sub(s).Hours()/24 > float64(chargesRangeMaxDays) {
		return time.Time{}, time.Time{}, false
	}
	return s, e, true
}
...
// buildChargesPresets builds the two named date-filter presets — "last 7
