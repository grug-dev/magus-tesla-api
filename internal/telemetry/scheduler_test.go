...
// stubCollector records how many times CollectAll ran so we can prove the scheduler
// stops cleanly without firing a cycle when its context is already cancelled.
type stubCollector struct{ calls int }

func (s *stubCollector) CollectAll(context.Context) (CycleReport, error) {
	s.calls++
...
func TestScheduler_ShutsDownWithoutRunningWhenCancelled(t *testing.T) {
	...
	// promptly without waiting for the timer and without calling CollectAll.
	sched := NewScheduler(sc, 3, 30, time.UTC, Config{})
...
func TestScheduler_NilLocationDefaultsToLocal(t *testing.T) {
	sched := NewScheduler(&stubCollector{}, 3, 30, nil, Config{})
	if sched.loc != time.Local {
		t.Errorf("nil location should default to time.Local, got %v", sched.loc)
	}
}

// reportCollector returns a canned report/error so the Run logging path is exercised.
...
func (c *reportCollector) CollectAll(context.Context) (CycleReport, error) {
	c.calls++
...
func TestScheduler_RunsAndLogsOneCycle(t *testing.T) {
	...
	rc := &reportCollector{
		report: CycleReport{Attempted: 3, Succeeded: 2, FailuresByReason: map[Reason]int{ReasonAPIError: 1}},
		err:    errors.New("cycle: boom"),
	}
	sched := NewScheduler(rc, 3, 30, time.UTC, Config{Clock: func() time.Time { return fixed }})
