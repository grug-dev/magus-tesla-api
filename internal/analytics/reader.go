...
func (r *reader) LatestMetricsForVehicles(ctx context.Context, refs []vehicleref.Ref) ([]VehicleStatus, error) {
	...
	for _, row := range rows {
		out = append(out, VehicleStatus{
			...
			DistanceTraveledKmDeltaCalc: ptrFloat64FromPg(row.DistanceTraveledKmDeltaCalc),
			ConsumedPctDeltaCalc:        ptrFloat64FromPg(row.ConsumedPctDeltaCalc),
			KmPerPctDeltaCalc:           ptrFloat64FromPg(row.KmPerPctDeltaCalc),
			TpmsPressureFLPSIDeltaCalc:  ptrFloat64FromPg(row.TpmsPressureFlPsiDeltaCalc),
			TpmsPressureFRPSIDeltaCalc:  ptrFloat64FromPg(row.TpmsPressureFrPsiDeltaCalc),
			TpmsPressureRLPSIDeltaCalc:  ptrFloat64FromPg(row.TpmsPressureRlPsiDeltaCalc),
			TpmsPressureRRPSIDeltaCalc:  ptrFloat64FromPg(row.TpmsPressureRrPsiDeltaCalc),
		})
	}
	return out, nil
}
